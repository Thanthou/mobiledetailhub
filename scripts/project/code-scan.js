 ...
 * 🔍 Code Scan Utility
 * Usage:
 *   node scripts/project/code-scan.js --scripts
 *   node scripts/project/code-scan.js --frontend/src
 *
 * Features:
 *  - Recursively scans all text-based files in target directory
 *  - Skips binary / irrelevant files
 *  - Creates a JSON file tree
 *  - Splits output into 512MB chunks
 *  - Outputs to docs/code-scan/
 */
...
if (!arg) {
  console.error("❌ Missing argument. Example: node code-scan.js --scripts or --frontend/src");
  process.exit(1);
}
...
const OUTPUT_DIR = path.join(ROOT_DIR, "docs/code-scan");
...
// 🚀 Run
// -----------------------------
console.log(`🔍 Starting code scan for: ${TARGET_DIR}`);
