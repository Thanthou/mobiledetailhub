...
/**
 * Database Migration Manager (CommonJS Version)
 * 
 * Manages database migrations with proper naming conventions and tracking
 * 
 * Usage:
 *   node scripts/migrate-commonjs.cjs status          # Show migration status
 *   node scripts/migrate-commonjs.cjs up              # Run all pending migrations
 *   node scripts/migrate-commonjs.cjs down            # Rollback last migration
 *   node scripts/migrate-commonjs.cjs create <name>   # Create new migration
 *   node scripts/migrate-commonjs.cjs reset           # Reset all migrations
 */
...
function parseMigrationFile(filename) {
  // Support both formats: YYYYMMDDHHMMSS_description.sql and YYYY-MM-DD_HHMM_description.sql
  let match = filename.match(/^(\d{14})_(.+)\.sql$/);
  if (!match) {
    // Try the alternative format with dashes
    match = filename.match(/^(\d{4}-\d{2}-\d{2}_\d{4})_(.+)\.sql$/);
    if (!match) {
      return null;
    }
    // Convert YYYY-MM-DD_HHMM to YYYYMMDDHHMM00 for consistency
    const [, dateTime, description] = match;
    const timestamp = dateTime.replace(/[-_:]/g, '').substring(0, 12) + '00';
    return {
      filename,
...
async function initializeMigrationTable() {
  ...
  try {
    // First ensure we have a public schema
    ...
    switch (command) {
      ...
      case 'create':
        if (!arg) {
          ...
          console.log('Usage: node migrate-commonjs.cjs create "add user authentication"');
          process.exit(1);
        }
        createMigration(arg);
        break;
      ...
      default:
        ...
        console.log('Usage:');
        console.log('  node migrate-commonjs.cjs status          # Show migration status');
        console.log('  node migrate-commonjs.cjs up              # Run all pending migrations');
        console.log('  node migrate-commonjs.cjs down            # Rollback last migration');
        console.log('  node migrate-commonjs.cjs create <name>   # Create new migration');
        console.log('  node migrate-commonjs.cjs reset           # Reset all migrations');
        break;
    }
  } catch (error) {
