...
// Log level is set by env.async.js - no need to set here
...
// .env loading happens silently via dotenv.config() in env.async.js

// Environment variables loaded by env.async.js
...
// Environment loaded and validated silently
...
try {
  ...
  logger.debug('Analytics router initialized');
} catch (error) {
...
// ✅ Body parsers FIRST - must come before any middleware that might read request
...
// Routes loaded silently - enable DEBUG log level to see route registration
...
// 404 handler for API routes that don't match (must come after all API route definitions)
// This will only fire if no previous API route handled the request
app.use((req, res, next) => {
...
const assetsPath = env.NODE_ENV === 'production' 
...
const adminPath = env.NODE_ENV === 'production'
  ? path.join(__dirname, 'public', 'admin')
  : path.join(__dirname, '../frontend/dist/apps/admin-app');
...
const tenantPath = env.NODE_ENV === 'production'
  ? path.join(__dirname, 'public', 'tenant')
  : path.join(__dirname, '../frontend/dist/apps/tenant-app');
...
const mainPath = env.NODE_ENV === 'production'
  ? path.join(__dirname, 'public', 'main')
  : path.join(__dirname, '../frontend/dist/apps/main');
...
app.get('*', (req, res) => {
  ...
  if (requestPath.startsWith('/admin')) {
    const htmlFile = env.NODE_ENV === 'production' 
      ? path.join(__dirname, 'public/admin/index.html')
      : path.join(__dirname, '../frontend/dist/apps/admin-app/index.html');
  ...
  if (requestPath.startsWith('/tenant')) {
    const htmlFile = env.NODE_ENV === 'production' 
      ...
      : path.join(__dirname, '../frontend/dist/apps/tenant-app/index.html');
    return res.sendFile(htmlFile);
  }
  ...
  const htmlFile = env.NODE_ENV === 'production' 
    ...
    : path.join(__dirname, '../frontend/dist/apps/main/index.html');
  
  res.sendFile(htmlFile);
});

// Static port configuration (no dynamic port detection)
const PORT = env.BACKEND_PORT || env.PORT || 3001;
const HOST = '0.0.0.0'
...
async function initializeAsync() {
  ...
  try {
    ...
    updateOAuthRedirect()
    logger.debug('OAuth redirect URI updated')
  } catch (error) {
    logger.debug('Could not update OAuth redirect URI:', error.message)
  }
  ...
  try {
    ...
    logger.debug('Could not initialize health monitor:', error.message)
  }
  ...
  try {
    ...
    logger.debug('Could not initialize cron jobs:', error.message)
  }
}
