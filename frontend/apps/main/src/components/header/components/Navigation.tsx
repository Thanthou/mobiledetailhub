...
const Navigation: React.FC<NavigationProps> = ({ activeSection }) => {
  ...
  const getNavItemClasses = (link: typeof NAV_LINKS[0]) => {
    ...
    const baseClasses = "nav-link hover:text-cyan-400 focus:text-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2 focus:ring-offset-black/20 transition-colors duration-200 p-2 font-inherit cursor-pointer rounded";
    
    const finalClasses = isLinkActive 
      ? `${baseClasses} text-cyan-400 bg-transparent border-none ring-2 ring-cyan-400 ring-offset-2 ring-offset-black/20` 
