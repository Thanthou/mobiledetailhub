...
const QuoteModal: React.FC<QuoteModalProps> = ({ isOpen, onClose }) => {
  ...
  // Type guard for affiliate data with base_location
  const isAffiliateDataWithLocation = (data: unknown): data is { base_location: { city: string; state_name: string } } => {
    return (
      data !== null &&
      typeof data === 'object' &&
      'base_location' in data &&
      data.base_location !== null &&
      typeof data.base_location === 'object' &&
      'city' in data.base_location &&
      'state_name' in data.base_location &&
      typeof data.base_location.city === 'string' &&
      typeof data.base_location.state_name === 'string'
    );
  };
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (selectedLocation && selectedLocation.city && selectedLocation.state) {
        locationToSet = `${String(selectedLocation.city)}, ${String(selectedLocation.state)}`;
      } 
      ...
      else if (isAffiliateDataWithLocation(affiliateData)) {
