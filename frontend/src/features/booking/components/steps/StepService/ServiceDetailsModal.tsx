...
const ServiceDetailsModal: React.FC<ServiceDetailsModalProps> = ({
  ...
  // Load features data based on vehicle type
  useEffect(() => {
    if (isOpen && bookingData.vehicle) {
      loadFeaturesForVehicle(bookingData.vehicle);
    }
  }, [isOpen, bookingData.vehicle]);

  const loadFeaturesForVehicle = async (vehicleType: string) => {
    try {
      ...
      console.error(`❌ Error loading features for ${vehicleType}:`, error);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-stone-900 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          {/* Description */}
          <div className="mb-6">
            <p className="text-stone-300 text-lg">{service.description}</p>
          </div>

          {/* Features */}
          <div className="space-y-4">
            <h3 className="text-xl font-semibold text-white mb-4">What's Included:</h3>
            {service.featureIds.map((featureId) => {
