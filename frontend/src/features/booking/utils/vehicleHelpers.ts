...
import { vehicleData } from '../data/vehicles.data';
...
export const getAvailableMakes = (type: string): string[] => {
  ...
  const data = vehicleData[type as keyof typeof vehicleData];
  if (Array.isArray(data)) {
    // Car data structure
    return data.map(item => item.brand);
  } else {
    // Boat/RV data structure
...
export const getAvailableModels = (type: string, make: string): string[] => {
  ...
  if (Array.isArray(data)) {
    // Car data structure
    const car = data.find(item => item.brand === make);
    return car ? car.models : [];
  } else {
    // Boat/RV data structure
    return (data as Record<string, string[]>)[make] || [];
  }
};
