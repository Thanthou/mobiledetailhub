...
const InteriorExteriorModal: React.FC<InteriorExteriorModalProps> = ({ isOpen, onClose, onBookNow, onRequestQuote }) => {
  ...
  const nextVideo = () => {
    setCurrentVideoIndex((prev) => (prev + 1) % interiorExteriorService.videos.length);
  };

  const prevVideo = () => {
    setCurrentVideoIndex((prev) => (prev - 1 + interiorExteriorService.videos.length) % interiorExteriorService.videos.length);
  };
