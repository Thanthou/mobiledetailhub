...
import { X, ChevronLeft, ChevronRight, Shield, Image as ImageIcon } from 'lucide-react';
...
const CeramicCoatingModal: React.FC<CeramicCoatingModalProps> = ({ isOpen, onClose, onBookNow, onRequestQuote }) => {
  ...
  const nextVideo = () => {
    setCurrentVideoIndex((prev) => (prev + 1) % ceramicCoatingService.videos.length);
  };

  const prevVideo = () => {
    setCurrentVideoIndex((prev) => (prev - 1 + ceramicCoatingService.videos.length) % ceramicCoatingService.videos.length);
  };
                ...
                {validImages.length > 0 ? (
                  <div className="relative">
                    <img
                      ...
                      alt={`${ceramicCoatingService.title} ${currentImageIndex + 1}`}
